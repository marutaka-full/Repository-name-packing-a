...
def extract_articles(node, out=None):
    """
    ツリー内の tag=='Article' を階層に関わらず全て見つけ、
    {条番号(attr.Num): 正規化本文} の順序付き辞書を返す。
    """
    if out is None:
        out = OrderedDict()
    if isinstance(node, dict):
        if node.get("tag") == "Article":
            ...
            out[num] = normalize(walk_text(node))
            return out
        for ch in node.get("children", []):
            extract_articles(ch, out)
...
def article_sort_key(num):
    """'2', '10', '2_2'（第二条の二）等を自然順で並べる。"""
    parts = re.split(r"[^0-9]+", str(num))
...
def save_current(law_id, name, art_map):
    ...
    for num in sorted(art_map, key=article_sort_key):
        lines.append(art_map[num])
        lines.append("")
